...
func (r *Settings) SetReporterStatus(status *core.ReporterStatus) {
	r.StatusOneof = &Settings_ReporterStatus{ReporterStatus: status}
}
...
func (r *Settings) UpsertReporterStatus(status *core.Status) {
...
// controller.
// Note: POD_NAMESPACE environment variable must be set for this function to behave as expected.
func (r *Settings) GetNamespacedStatus() *core.Status {
	podNamespace := os.Getenv("POD_NAMESPACE")
	if podNamespace != "" {
		if r.GetReporterStatus() == nil {
			return nil
...
func (r *Settings) HasReporterStatus() bool {
...
func (r *Settings) HasStatus() bool {
	switch r.StatusOneof.(type) {
