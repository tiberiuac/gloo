...
func (r *AuthConfig) SetNamespacedStatuses(status *core.NamespacedStatuses) {
	r.StatusOneof = &AuthConfig_NamespacedStatuses{NamespacedStatuses: status}
}
...
func (r *AuthConfig) UpsertNamespacedStatus(status *core.Status) error {
...
// Note: POD_NAMESPACE environment variable must be set for this function to behave as expected.
// If unset, a podNamespaceErr is returned.
func (r *AuthConfig) GetNamespacedStatus() (*core.Status, error) {
	podNamespace := os.Getenv("POD_NAMESPACE")
	if podNamespace == "" {
