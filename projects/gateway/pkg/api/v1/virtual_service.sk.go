...
func (r *VirtualService) SetReporterStatus(status *core.ReporterStatus) {
	r.StatusOneof = &VirtualService_ReporterStatus{ReporterStatus: status}
}
...
func (r *VirtualService) AddToReporterStatus(status *core.Status) {
...
// Note: POD_NAMESPACE environment variable must be set for this function to behave as expected.
func (r *VirtualService) GetStatusForReporter(reportedBy string) *core.Status {
	podNamespace := os.Getenv("POD_NAMESPACE")
	if podNamespace != "" {
...
func (r *VirtualService) HasReporterStatus() bool {
...
func (r *VirtualService) HasStatus() bool {
	switch r.StatusOneof.(type) {
