...
func (r *VirtualHostOption) SetReporterStatus(status *core.ReporterStatus) {
	r.StatusOneof = &VirtualHostOption_ReporterStatus{ReporterStatus: status}
}
...
func (r *VirtualHostOption) UpsertReporterStatus(status *core.Status) {
...
// controller.
// Note: POD_NAMESPACE environment variable must be set for this function to behave as expected.
func (r *VirtualHostOption) GetNamespacedStatus() *core.Status {
	podNamespace := os.Getenv("POD_NAMESPACE")
	if podNamespace != "" {
		if r.GetReporterStatus() == nil {
			return nil
...
func (r *VirtualHostOption) HasReporterStatus() bool {
...
func (r *VirtualHostOption) HasStatus() bool {
	switch r.StatusOneof.(type) {
