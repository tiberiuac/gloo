...
func (r *RouteTable) SetNamespacedStatuses(status *core.NamespacedStatuses) {
	r.StatusOneof = &RouteTable_NamespacedStatuses{NamespacedStatuses: status}
}
...
func (r *RouteTable) UpsertNamespacedStatus(status *core.Status) error {
...
// Note: POD_NAMESPACE environment variable must be set for this function to behave as expected.
// If unset, a podNamespaceErr is returned.
func (r *RouteTable) GetNamespacedStatus() (*core.Status, error) {
	podNamespace := os.Getenv("POD_NAMESPACE")
	if podNamespace == "" {
